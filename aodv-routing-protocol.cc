  ...
  if (m_routingTable.LookupValidRoute (dst, rt))
    {
      route = rt.GetRoute ();
      NS_ASSERT (route != 0);
      NS_LOG_INFO ("Exist route to " << route->GetDestination () << " from interface " << route->GetSource ());
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      if (lcb.IsNull () == false)
        {
          NS_LOG_INFO ("Unicast local delivery to " << dst);
          //CNLAB
      ...
      if (!socket)
        {
          ...
          // create also a subnet directed broadcast socket. Added. CNLAB
          socket = Socket::CreateSocket (GetObject<Node> (),
                                                       UdpSocketFactory::GetTypeId ());
          NS_ASSERT (socket != 0);
          socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv, this));
      ...
      if (l3->GetNAddresses (i))
        {
          ...
          NS_ASSERT (socket != 0);
          socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv, this));
          // Bind to any IP address so that broadcasts can be received. TTL Added. CNLAB
          socket->Bind (InetSocketAddress (iface.GetLocal (), AODV_PORT));
          socket->BindToNetDevice (l3->GetNetDevice (i));
          socket->SetAllowBroadcast (true);
          socket->SetAttribute ("IpTtl", UintegerValue (1));
          ...
          // create also a unicast socket. Added. CNLAB.
          socket = Socket::CreateSocket (GetObject<Node> (),
                                                       UdpSocketFactory::GetTypeId ());
          NS_ASSERT (socket != 0);
          socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv, this));
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  NS_LOG_INFO ("AODV node " << this << " received a AODV packet from " << sender << " to " << receiver);
  ...
  for (std::vector<Ipv4InterfaceAddress>::const_iterator i = ifaces.begin (); i != ifaces.end (); ++i)
    {
      ...
      // std::cout << "Broadcast RERR message from interface " << i->GetLocal () << std::endl; //added. CNLAB
      // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
      Ptr<Packet> p = packet->Copy (); //added. CNLAB
      Ipv4Address destination;
